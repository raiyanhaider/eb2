...
export async function generateStaticParams() {
  const feature = FEATURES.find(f => f.id === 'character-profile');
  if (!feature) return [];
...
export default function CharacterProfileStepPage({ 
  params 
}: { 
  params: { step: string } 
}) {
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <div className="flex justify-between">
              {prevStep ? (
                <Link href={`/character-profile/steps/${prevStep}`}>
              ...
              {nextStep && (
                <Link href={`/character-profile/steps/${nextStep}`}>
                  <Button>
                    Next Step
