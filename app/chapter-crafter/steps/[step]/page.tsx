...
export async function generateStaticParams() {
  const feature = FEATURES.find(f => f.id === 'chapter-crafter');
  if (!feature) return [];
...
export default function ChapterCrafterStepPage({ 
  params 
}: { 
  params: { step: string } 
}) {
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <div className="flex justify-between">
              {prevStep ? (
                <Link href={`/chapter-crafter/steps/${prevStep}`}>
              ...
              {nextStep && (
                <Link href={`/chapter-crafter/steps/${nextStep}`}>
                  <Button>
                    Next Step
