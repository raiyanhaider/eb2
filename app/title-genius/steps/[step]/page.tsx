...
export async function generateStaticParams() {
  const feature = FEATURES.find(f => f.id === 'title-genius');
  if (!feature) return [];
...
export default function TitleGeniusStepPage({ 
  params 
}: { 
  params: { step: string } 
}) {
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <div className="flex justify-between">
              {prevStep ? (
                <Link href={`/title-genius/steps/${prevStep}`}>
              ...
              {nextStep && (
                <Link href={`/title-genius/steps/${nextStep}`}>
                  <Button>
                    Next Step
