...
export async function generateStaticParams() {
  const feature = FEATURES.find(f => f.id === 'auto-chapter');
  if (!feature) return [];
...
export default function AutoChapterStepPage({ 
  params 
}: { 
  params: { step: string } 
}) {
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <div className="flex justify-between">
              {prevStep ? (
                <Link href={`/auto-chapter/steps/${prevStep}`}>
              ...
              {nextStep && (
                <Link href={`/auto-chapter/steps/${nextStep}`}>
                  <Button>
                    Next Step
