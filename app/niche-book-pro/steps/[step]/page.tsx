...
export async function generateStaticParams() {
  const feature = FEATURES.find(f => f.id === 'niche-book-pro');
  if (!feature) return [];
...
export default function NicheBookProStepPage({ 
  params 
}: { 
  params: { step: string } 
}) {
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <div className="flex justify-between">
              {prevStep ? (
                <Link href={`/niche-book-pro/steps/${prevStep}`}>
              ...
              {nextStep && (
                <Link href={`/niche-book-pro/steps/${nextStep}`}>
                  <Button>
                    Next Step
