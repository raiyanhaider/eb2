...
export async function generateStaticParams() {
  const feature = FEATURES.find(f => f.id === 'perfect-prose');
  if (!feature) return [];
...
export default function PerfectProseStepPage({ 
  params 
}: { 
  params: { step: string } 
}) {
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <div className="flex justify-between">
              {prevStep ? (
                <Link href={`/perfect-prose/steps/${prevStep}`}>
              ...
              {nextStep && (
                <Link href={`/perfect-prose/steps/${nextStep}`}>
                  <Button>
                    Next Step
