...
export default function FeatureStepPage({ 
  ...
  params: { feature: string; step: string } 
}) {
  const feature = FEATURES.find(f => f.id === params.feature);
  const currentStepIndex = feature?.steps.indexOf(params.step) ?? -1;
  
  if (!feature || currentStepIndex === -1) {
    notFound();
  }
            ...
            <div className="flex justify-between">
              {prevStep ? (
                <Button
                  variant="outline"
                  as={Link}
                  href={`/${feature.id}/steps/${prevStep}`}
                >
              ...
              {nextStep && (
                <Button
                  as={Link}
                  href={`/${feature.id}/steps/${nextStep}`}
                >
                  Next Step
