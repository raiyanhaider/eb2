...
export async function generateStaticParams() {
  const feature = FEATURES.find(f => f.id === 'story-sculptor');
  if (!feature) return [];
...
export default function StorySculptorStepPage({ 
  params 
}: { 
  params: { step: string } 
}) {
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <div className="flex justify-between">
              {prevStep ? (
                <Link href={`/story-sculptor/steps/${prevStep}`}>
              ...
              {nextStep && (
                <Link href={`/story-sculptor/steps/${nextStep}`}>
                  <Button>
                    Next Step
