...
export async function generateStaticParams() {
  const feature = FEATURES.find(f => f.id === 'content-expander');
  if (!feature) return [];
...
export default function ContentExpanderStepPage({ 
  params 
}: { 
  params: { step: string } 
}) {
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <div className="flex justify-between">
              {prevStep ? (
                <Link href={`/content-expander/steps/${prevStep}`}>
              ...
              {nextStep && (
                <Link href={`/content-expander/steps/${nextStep}`}>
                  <Button>
                    Next Step
